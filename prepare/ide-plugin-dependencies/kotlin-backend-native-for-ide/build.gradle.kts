...
idePluginDependency {
    ...
    val proguard by task<CacheableProguardTask> {
        val packageName = kotlinEmbeddableRootPackage
        dependsOn(shadowJar)

        buildDir.mkdirs()
        val config = fileFrom(buildDir, "backend-native-for-ide.pro")
        config.writeText(
            "${fileFrom(projectDir, "backend-native-for-ide.pro").readText()}\n" +
